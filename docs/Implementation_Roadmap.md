...
**Milestone 6 scope note 2 (added during M6 frontend implementation):** `GET /exams/{id}/submissions/{submission_id}` (Derived Engineering Addition, confirmed with the user) is required for the Teacher Grading Interface (`UI_Wireframes.md` §14) to function at all — `POST /exams/{id}/grade` requires `answer_id` values but no existing endpoint returned a submission's actual answer content alongside those ids, and `GET /exams/{id}/results` is deliberately aggregate-only (reporting, not grading). See `API_Contract.md` §3.8 and `Proposal_vs_Engineering_Additions.md`.
...
- `GET /exams/{id}/submissions/{submission_id}` *(Derived addition, confirmed with the user during M6 frontend implementation — see the scope note above; `API_Contract.md` §3.8)*
- `POST /exams/{id}/grade`
...
**Milestone 7 scope note 2 (added during M7 implementation, confirmed with the user):** `GET /results/pending` (Derived Engineering Addition) is required for the Admin: Result Approval page (`UI_Wireframes.md` §11) to function at all — none of the milestone's other four endpoints can list/retrieve pending results, and `GET /results/reports` (§9.1) is a different, aggregate/published-only reporting endpoint out of Milestone 7's scope. See `API_Contract.md` §5.3 and `Proposal_vs_Engineering_Additions.md`.
...
- `GET /results/pending` *(Derived addition, confirmed with the user during M7 implementation — see the scope note above; `API_Contract.md` §5.3)*

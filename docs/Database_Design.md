...
| Unique constraint | (student_id, fee_structure_id) | one invoice per student per fee structure — Milestone 8's duplicate-prevention rule |

**Design note (added during the Milestone 8 pre-implementation review — invoice creation):** the original draft of this table, and `API_Contract.md`'s documented endpoint list, had no mechanism for creating an `invoice` row at all — `fee_structure` is department/semester-scoped, not student-scoped, and `POST /fees/payments` only *recalculates* an invoice's status (per `API_Contract.md` §6.3), it doesn't create one. But the Fee Centre wireframe (`UI_Wireframes.md` §8) shows a nonzero "Outstanding Balance" before any row exists in Payment History, meaning an unpaid invoice must already exist independent of any payment. Presented to the user via `AskUserQuestion`; the user approved: `POST /fees` (creating a `fee_structure`) automatically generates one `unpaid` invoice for every currently-**active** student who has **≥1 `Enrollment`** in a `class_session` for that `fee_structure.semester_id`, and whose own `student.department_id` matches `fee_structure.department_id` (or every such student, if `department_id` is null/university-wide). This is documented behavior of the existing `POST /fees` endpoint, not a new endpoint. `(student_id, fee_structure_id)` is the duplicate-prevention key — a student is invoiced for a given fee structure exactly once, at generation time.

**Design note (`status` recalculation, resolves NFR-016 "computed on demand, not manually recalculated"):** `status` is written by the service layer immediately whenever a payment is recorded against that invoice (never a separate manual recalculation step) — `unpaid` → `partially_paid` → `paid` as cumulative payments approach `fee_structure.amount`. The `overdue` value is never written to this column; it is computed at read time (`GET /fees/me`, `GET /fees/overdue`) as `status in (unpaid, partially_paid) AND due_date < today` — the same "never cache a value that's a function of today's date" philosophy already applied to attendance percentage and GPA (`CLAUDE.md` §11).

**Design note (`pdf_url`, unused):** consistent with Milestone 7's transcript generation (`app/pdf/transcript_generator.py`), invoice PDFs are generated on demand at `GET /fees/invoices/{id}` request time via `reportlab`, not pre-generated and stored — there is no file-storage layer in this project to point `pdf_url` at. This column stays permanently `null`; it is not populated or read by any Milestone 8 code.

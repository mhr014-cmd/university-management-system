...
| FR-031 | System issues low-attendance warnings automatically | High | Student | Attendance | `attendance_record`, `notification` | (derived server-side; surfaces via `GET /attendance/me`; actual dispatch via the `notification` module, Milestone 9) | Attendance page, Notifications panel | BR-008 (threshold: 80%, resolved — Requirement_Analysis.md §14 item 4) | Pending | Pending | Pending |

...
|---|---|---|---|---|---|---|---|---|---|---|---|
| FR-001 | Log in with email and password; returns access + refresh token | Critical | All | Authentication & Authorization | `user` | `POST /auth/login` | Login | VR-001 | Pending | Pending | Pending |
| FR-002 | Refresh an expired access token without re-entering credentials | Critical | All | Authentication & Authorization | `user` | `POST /auth/refresh` | Login (silent refresh, no dedicated screen) | — | Pending | Pending | Pending |
| FR-003 | Log out and invalidate current session | High | All | Authentication & Authorization | `user` | `POST /auth/logout` | Dashboard (logout action; no dedicated screen) | — | Pending | Pending | Pending |
| FR-004 | Change own password | High | All | Authentication & Authorization | `user` | `PUT /auth/password` | Profile page | VR-002 | Pending | Pending | Pending |
| FR-005 | Redirect to role-specific dashboard after successful login | High | All | Authentication & Authorization | `user` | `POST /auth/login` (redirect logic is frontend-side) | Dashboard | — | Pending | Pending | Pending |
| FR-006 | Retrieve own profile | Critical | All | User Management | `user`, `student`, `teacher`, `parent`, `admin` | `GET /users/me` | Profile page | — | Pending | Pending | Pending |
| FR-007 | Update own profile (personal info, profile photo) | Critical | All | User Management | `user`, `student`, `teacher`, `parent`, `admin` | `PUT /users/me` | Profile page | VR-009 | Pending | Pending | Pending |
| FR-008 | Student views academic history alongside profile data | High | Student | User Management | `student`, `enrollment`, `result` | `GET /users/me` | Profile page | — | Pending | Pending | Pending |
| FR-009 | Admin lists all students | Critical | Admin | User Management | `student`, `user` | `GET /users/students` | Admin: user management | — | Pending | Pending | Pending |
| FR-010 | Admin creates a new student account | Critical | Admin | User Management | `student`, `user` | `POST /users/students` | Admin: user management | VR-001 (email format) | Pending | Pending | Pending |
| FR-011 | Admin or Teacher retrieves a single student's record | Critical | Admin, Teacher | User Management | `student`, `user` | `GET /users/students/{id}` | Admin: user management | — | Pending | Pending | Pending |
| FR-012 | Admin updates a student's record | Critical | Admin | User Management | `student`, `user` | `PUT /users/students/{id}` | Admin: user management | VR-009 | Pending | Pending | Pending |
| FR-013 | Admin deactivates a student account (soft delete) | Critical | Admin | User Management | `student`, `user` | `DELETE /users/students/{id}` | Admin: user management | BR-006 | Pending | Pending | Pending |
| FR-014 | Admin lists all teachers | Critical | Admin | User Management | `teacher`, `user` | `GET /users/teachers` | Admin: user management | — | Pending | Pending | Pending |
| FR-015 | Admin creates a new teacher account | Critical | Admin | User Management | `teacher`, `user` | `POST /users/teachers` | Admin: user management | VR-001 (email format) | Pending | Pending | Pending |
| FR-016 | Admin updates a teacher's record | Critical | Admin | User Management | `teacher`, `user` | `PUT /users/teachers/{id}` | Admin: user management | VR-009 | Pending | Pending | Pending |
...
| FR-045 | Student or Teacher views own timetable | Critical | Student, Teacher | Scheduling | `schedule_entry`, `class_session` | `GET /schedule/me` | Timetable | — | Pending | Pending | Pending |
| FR-046 | Admin creates a class schedule entry | Critical | Admin | Scheduling | `schedule_entry` | `POST /schedule` | (Admin schedule management — not separately listed as a screen in Requirement_Analysis.md §7; implied within Timetable/Admin scope) | VR-007 | Pending | Pending | Pending |
| FR-047 | Admin updates a schedule entry | Critical | Admin | Scheduling | `schedule_entry` | `PUT /schedule/{id}` | (Admin schedule management — see FR-046 note) | VR-007 | Pending | Pending | Pending |
| FR-048 | Admin removes a class from the schedule | Critical | Admin | Scheduling | `schedule_entry` | `DELETE /schedule/{id}` | (Admin schedule management — see FR-046 note) | — | Pending | Pending | Pending |
| FR-049 | Admin detects scheduling conflicts (double-booked rooms/teachers) before publishing | Critical | Admin | Scheduling | `schedule_entry`, `room`, `teacher` | `GET /schedule/conflicts` | (Admin schedule management — see FR-046 note) | BR-005 | Pending | Pending | Pending |
| FR-050 | Teacher requests a timetable change, routed to Admin for confirmation | High | Teacher, Admin | Scheduling | `schedule_change_request` | (no dedicated endpoint listed in proposal §6 — gap, see Requirement_Analysis.md §8 note) | Timetable (Teacher request action) | BR-004 | Pending | Pending | Pending |
| FR-051 | System sends instant notifications when a student's schedule changes | High | Student | Scheduling / Notification | `schedule_entry`, `notification` | (derived server-side; surfaces via `notification` module) | Timetable, Notifications panel | — | Pending | Pending | Pending |
...
| Req ID | Requirement Description | Priority | User Role | Module | Database Tables | Backend API | Frontend Pages | Validation Rules | Testing Status | Implementation Status | Verification Status |
|---|---|---|---|---|---|---|---|---|---|---|---|
| NFR-001 | Enforce role-based access control at the API layer for every endpoint, not only in the UI | Critical | All | Authentication & Authorization (cross-cutting) | `user` | All protected endpoints (RBAC middleware, `System_Architecture.md` §6) | All pages (client-side hiding is UX only) | — | Pending | Pending | Pending |
...
| NFR-004 | Use JWT bearer tokens with short-lived access tokens and refresh-token rotation | Critical | All | Authentication & Authorization | `user` | `POST /auth/login`, `POST /auth/refresh`, `POST /auth/logout` | Login | — | Pending | Pending | Pending |
...
| NFR-015 | Scheduling subsystem prevents double-booking of rooms/teachers at creation/update time | Critical | Admin | Scheduling | `schedule_entry` | `POST /schedule`, `PUT /schedule/{id}`, `GET /schedule/conflicts` | Timetable | BR-005 | Pending | Pending | Pending |
...
- **FR-050, FR-052, FR-053, NFR-009**: No notification-management endpoints (`GET /notifications`, `PUT /notifications/{id}/read`) or schedule-change-request endpoint are listed in the proposal's §6 API spec, despite being required by the corresponding features. These are marked with a gap note rather than a fabricated endpoint; `Implementation_Roadmap.md` Milestone 9 proposes the missing endpoints as an implementation necessity.

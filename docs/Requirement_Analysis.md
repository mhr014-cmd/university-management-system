...
6. ~~**GPA calculation formula is not defined**~~ **Resolved during the Milestone 7 pre-implementation review**, per this document's own A-004 assumption ("a conventional university GPA scheme" — credit-hour-weighted). `GET /results/me`'s `gpa` per semester = `sum(grade_point * course.credit_hours) / sum(course.credit_hours)` across that semester's `published` results, using `course.credit_hours` (already in `Database_Design.md` §6.8) as the weight. `grade_letter`/`grade_point` themselves are not computed by the system from raw exam marks — per `API_Contract.md` §5.2, the submitting Teacher supplies both directly per student, so no letter-grade-boundary formula is invented or hard-coded anywhere.

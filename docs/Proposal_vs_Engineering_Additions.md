...
### Report printing / PDF / Excel export infrastructure (Version 1.2)
**Where:** `backend/app/pdf/shared.py`, `backend/app/pdf/attendance_report.py`, `backend/app/excel/shared.py`, `backend/app/excel/attendance_report.py`, `GET /attendance/reports/pdf`, `GET /attendance/reports/excel`, `frontend/src/components/ui/ReportLayout.tsx`, `frontend/src/components/ui/ReportToolbar.tsx`, `frontend/src/lib/usePrint.ts`, `frontend/src/lib/exportClient.ts`, `frontend/src/styles/print.css`.
**Classification: Design Enhancement.** Nothing in `product_proposal.pdf`, `Requirement_Analysis.md`, `System_Architecture.md`, or `UI_Wireframes.md` mentions printing, PDF export, or Excel export for the Admin Reports screen — the proposal's Admin "Reports" feature (§5) only asks for the reports to be *generated* (already satisfied by FR-030/FR-054/FR-055's JSON endpoints), not exported to a document format. This capability was explicitly requested for Version 1.2 as a reusable reporting-infrastructure layer, built as a full vertical slice against Attendance Reports (the reference implementation) rather than speculative, uncalled generator functions: a shared reportlab-based PDF layout module and a shared openpyxl-based Excel layout module (both intentionally *not* a single over-generic `generate_tabular_report(...)` API — each report gets its own small module composing the shared helpers, so report-specific rendering stays local while layout/styling/header-footer/table code is reused), a shared frontend `ReportToolbar`/`ReportLayout` pair, a print stylesheet (A4, print-isolation via `[data-print-region]`), and a shared `exportClient` utility (`lib/exportClient.ts`) centralizing the "request a file, then trigger a browser download" behavior so Results/Fees/Timetable/Users report exports can reuse it without duplicating blob/`Content-Disposition`/anchor-click handling. New backend dependency: `openpyxl` (pure Python, no external binary — same selection rationale already documented for `reportlab`). Both new endpoints are Admin-only (`require_roles("admin")`), identical RBAC to the existing `GET /attendance/reports`; exported filenames are timestamped (`attendance-report-YYYYMMDD-HHMMSS.{pdf,xlsx}`) so repeated exports never overwrite a previous download. The pre-existing `transcript_generator.py`/`invoice_generator.py` (US-Letter, their own established layout) are untouched — new reports render on A4 instead, since no paper size is specified anywhere in `docs/` for this new capability.
**Disposition:** Permanent. Results/Fees/Timetable/Users report exports are expected to reuse `app/pdf/shared.py`, `app/excel/shared.py`, `ReportToolbar`, `ReportLayout`, and `exportClient` as they're built in subsequent Version 1.2 work, rather than each hand-rolling their own generator/toolbar/download logic.

### Academic Setup — reference-data management screens + update/delete (Version 2.3)
**Where:** `backend/app/schemas/{department,course,room,semester}.py` (`*Update` schemas), `backend/app/repositories/reference_data_repository.py` (`delete()` methods), `backend/app/services/reference_data_service.py` (`update()`/`delete()` methods), `PUT`/`DELETE /departments/{id}`, `/courses/{id}`, `/rooms/{id}`, `/semesters/{id}`; `frontend/src/pages/Admin/AcademicSetup/{Departments,Courses,Rooms,Semesters}.tsx`, `AcademicSetupTabs.tsx`, `frontend/src/features/{courses,rooms}/index.ts` (new) and `departments`/`semesters` (extended with create/update/delete), `frontend/src/components/ui/SearchableSelect.tsx`.
**Classification: Derived** for the backend update/delete endpoints (a mechanical prerequisite for the management screens the proposal implies an Admin needs, per the same "Reference Data... gap-fill domain" rationale already logged for the original list/create endpoints in `API_Contract.md` §10) and **Design Enhancement** for the frontend screens/SearchableSelect themselves (no wireframe ever specified this screen — see the pre-V2.3 architecture review, which found Department/Semester had only read-only dropdowns and Course/Room had zero frontend presence at all, not even read-only). Closes that exact gap: one new "Academic Setup" Admin nav entry → four separate, bookmarkable pages (not a tabbed single page), each following the existing `Admin/UserManagement` list+modal+`ConfirmDialog` pattern. `SearchableSelect` (new shared component) replaces 8 raw-UUID text inputs across `Timetable/index.tsx`'s Admin schedule-management forms (`course_id`, `teacher_id`, `semester_id`, `room_id`, `student_id`) with type-to-filter dropdowns sourced from the reference-data/user-list endpoints — `class_session_id` fields are intentionally left as raw text, since no `GET /schedule/class-sessions` list endpoint exists (documented Milestone 4 limitation, out of this pass's scope). **Delete design decision:** hard `DELETE`, relying on the already-documented `ON DELETE RESTRICT` FK policy (`Database_Design.md` §10) to reject deletion of anything still referenced (translated to 409 by the service layer) — no `is_active` column added, no migration. Re-evaluated explicitly per the user's request before implementation: these four entities are catalog/lookup data, not identity records, so `Database_Design.md`'s "Deactivation, not Deletion" policy (scoped to `user.is_active`/Student/Teacher) does not apply; adding soft-delete would have required a new migration plus touching every repository across attendance/fee/exam/result/schedule/user services that reads these tables, with no requirement identified that actually needs it.
**Disposition:** Permanent.

### Production-readiness gap-closure pass (post-V2.3 audit)

A full-project audit (Teacher/Parent portals, Admin approval workflows, notifications, reporting, RBAC) found the system already largely production-correct — Teacher dashboard scoping, attendance's live-computed data flow, Exam Builder, and the core RBAC/ownership layer needed no changes. The following concrete gaps were closed; each reuses an existing endpoint/pattern rather than introducing new architecture.

**Where (backend):** `backend/app/notifications/dispatcher.py` (`notify_result_published`/`notify_schedule_change` extended to fan out to linked Parents; new `notify_schedule_change_request_resolved`), `backend/app/services/schedule_service.py` (`resolve_change_request` now dispatches a notification to the requesting Teacher on approve/reject; new `list_change_requests`), `backend/app/repositories/schedule_repository.py` (`list_change_requests`), `backend/app/schemas/schedule.py` (`ScheduleChangeRequestListEntry`/`Response`), `GET /schedule/change-requests` (new, Admin-only), `backend/app/services/attendance_service.py` (`get_reports` now accepts Parent + ownership check), `backend/app/csv/attendance_report.py` (new — CSV export, stdlib `csv` only), `GET /attendance/reports/csv` (new), `GET /attendance/reports`, `/reports/pdf`, `/reports/excel` (RBAC extended to Admin-or-Parent), `backend/app/services/result_service.py` (`get_transcript_data` — Parent ownership branch added), `backend/app/services/fee_service.py` (`get_invoice_data` — Parent ownership branch added), `GET /fees/invoices/{id}` (RBAC extended to include Parent), `backend/app/pdf/invoice_generator.py` (labels the document "Fee Receipt" instead of "Fee Invoice" once `status == "paid"`, same PDF — no separate receipt generator).

**Where (frontend):** `frontend/src/pages/Timetable/index.tsx` (`RequestChangeModal` gained an optional room-change `SearchableSelect`; new `PendingChangeRequestsPanel` — Admin approval queue UI, previously entirely missing despite the backend create/resolve endpoints existing), `frontend/src/features/schedule/index.ts` (`useScheduleChangeRequests`/`useResolveScheduleChangeRequest`), `frontend/src/pages/ResultsView/index.tsx` and `frontend/src/pages/FeeCentre/index.tsx` (both now branch by role — Timetable's existing pattern — with a dedicated `ParentResultsView`/`ParentFeeCentre`, replacing the previous dashboard-widget-only Parent experience), `frontend/src/pages/Attendance/index.tsx` (Parent view gained a `ReportToolbar` with PDF/Excel/CSV export + a scoped `[data-print-region]` for Print), `frontend/src/features/attendance/index.ts` (`useExportAttendanceReportCsv`), `frontend/src/components/ui/ReportToolbar.tsx` (optional `onExportCsv`/`isExportingCsv` props, additive), `frontend/src/pages/Dashboard/ParentDashboard.tsx` (`RecentNotificationsCard`, reusing the existing generic `useNotifications` hook), `frontend/src/components/AppLayout.tsx` (Results/Fee Centre nav links extended to Parent).

**Classification: Derived** throughout — every item closes a gap between an already-documented proposal promise (Parent visibility into "Results & schedule", attendance alerts, downloadable reports) or an already-half-built feature (change-request create/resolve existed with no admin-facing queue) and its actual implementation. No new domain, no new database table, no schema/migration change — confirmed by running `alembic upgrade head` against a fresh disposable database with zero drift.

**Explicitly not built, per the user's own scope instructions for this pass:**
- **Communication/Messaging module** (message teacher/department, view replies) — confirmed to not exist anywhere in the codebase and not in `product_proposal.pdf`/`Requirement_Analysis.md` at all; building it would be new scope, not gap-closure, so it was intentionally left out.
- **Progress Report generator** — no such report exists for any role today; also a new report type, not attempted.
- **Teacher Remarks on the Results page** — the `result` table has no remarks/comments column (`Database_Design.md` §6.21 — only `grade_letter`/`grade_point`); the only existing "teacher remarks" data anywhere is `question_grade.feedback`, a different granularity (per-question, tied to one exam submission) already surfaced separately in the exam-taking view. Folding it into the aggregated Results view would need new join/aggregation logic, not just exposing an existing field, so the condition "if an existing model already supports it" wasn't met at this level — left out rather than half-built.
- **Overall CGPA** — `GET /results/me` returns only per-semester GPA and no per-course `credit_hours`, so an accurate cross-semester CGPA can't be computed client-side; the Results page shows an explicit "Not available" placeholder instead of a potentially-wrong client-side approximation (same limitation on the pre-existing Student view, not Parent-specific).
- **"Request modification"** on the Admin approval queue — expressed as Reject + an optional comment (visible to the Teacher via the new resolution notification) rather than a third request status, since the `schedule_change_request_status` Postgres enum is exactly `pending`/`approved`/`rejected` and adding a fourth value would require an Alembic migration, which the "no redesign" instruction ruled out for this pass.
- **"Suggest available rooms"** on the change-request form — not built; the room picker reuses the existing `useRooms()` list without cross-referencing the conflict-detection query, since the backend already rejects a conflicting room/time choice at resolve time (409), making a client-side suggestion a nice-to-have rather than a correctness requirement.
...
## Reports Print/PDF/Excel Consistency + Post-Manual-Test Fixes (2026-07-10)

Manual testing of the Reports-module consistency enhancement (Results/Fees gaining the same Print/PDF/Excel exports Attendance already had) found three follow-up gaps, all closed in this pass.

### `GET /results/reports`, `GET /fees/reports` — `details` array (Issues 1 & 2)
**Where (backend):** `backend/app/schemas/report.py` (`ResultDetailEntry`, `FeeDetailEntry`, `details` field on `ResultsReportResponse`/`FeesReportResponse`), `backend/app/services/report_service.py` (`get_results_report`/`get_fees_report` — batch student-name lookup + per-result course/exam lookup, per-invoice detail row), `backend/app/pdf/results_report.py`/`fees_report.py` (new "Detailed Results"/"Itemized Fees" section), `backend/app/excel/shared.py` (`build_report_workbook` gained an optional `extra_sheets` parameter — additive, every pre-existing caller including Attendance omits it and is byte-for-byte unchanged), `backend/app/excel/results_report.py`/`fees_report.py` (new "Details" worksheet).
**Where (frontend):** `frontend/src/features/results/index.ts`/`fees/index.ts` (`ResultDetailEntry`/`FeeDetailEntry` types), `frontend/src/pages/Admin/Reports/index.tsx` (new detail table rendered below each summary — needed for **Print** specifically, since Print is `window.print()` on the visible DOM, not a separate generation path; PDF/Excel alone wasn't sufficient).
**Classification: Derived.** The summary-only Results/Fees reports (added in the prior pass) showed aggregate totals with no way to see which underlying academic records/invoices produced them — a real usability gap surfaced by the user's own manual testing, not a new report type. Reuses the exact rows `get_results_report`/`get_fees_report` already fetch (`ResultRepository.list_published_for_report`, `FeeRepository.list_invoices_for_report`) — no new query source, no schema/migration change. Per-row course/exam lookups follow the same pattern already established in `result_service._build_semester_entries` (individual `course_repo.get()` per row, not a new batch method, for consistency with that precedent).
**Disposition:** Permanent.

### Notification bell / "Latest Notifications" staleness after another user's action (Issue 3)
**Where:** `frontend/src/features/notifications/index.ts` (`useNotifications` gained `refetchInterval: 30_000`).
**Classification: Design Enhancement.** No WebSocket/live-push channel exists in this project (`System_Architecture.md`), so a notification created by one user's action (e.g. Admin's "Send Bulk Overdue Notice" dispatching a `fee_due` notification to an affected Student) was never seen by that Student's already-open session — the notification bell lives in `AppLayout`, a persistent component that never remounts, so its query never naturally refetched. Fixed with the same `refetchInterval` polling convention already used by `lib/useHealthCheck.ts` (`30_000`ms) — explicitly not WebSockets, per the user's own instruction to reuse existing architecture. Verified live: the bell/"Latest Notifications" widget correctly reflects a newly-dispatched notification on next fetch/navigation (React Query's default `staleTime: 0` refetches on every mount); the 30-second in-session poll firing while a tab stays open could not be independently demonstrated in this session's browser-automation tooling, since CDP-controlled tabs report `visibilityState: "hidden"`/unfocused (Chrome throttles `setInterval`-based polling on background tabs by design) — this is a testing-environment limitation, not a code defect, and matches documented React Query/browser behavior.
**Disposition:** Permanent.

...
> **Known Limitation (final, recorded at Milestone 11):** this page was never implemented, and — with Milestone 11 being the final milestone in `Implementation_Roadmap.md` — will not be. No endpoint anywhere in `API_Contract.md` enumerates a Parent's linked children (only per-request ownership *verification* against `parent_student_link`, via a caller-supplied `student_id`); building the "list my children" endpoint this page's child selector depends on would be new backend functionality, which is explicitly out of scope for Milestone 11 ("Do NOT add new business functionality"). The gap was first identified in Milestone 7 and re-confirmed at every subsequent milestone through Milestone 10 (see `PROJECT_PROGRESS.md`'s M7–M10 Known Issues). The orphaned placeholder file (`frontend/src/pages/Parent/ChildView/index.tsx`) was removed during Milestone 11's dead-code cleanup — this wireframe remains as the recorded intended design, not as a description of implemented behavior. FR-032/FR-037/FR-038/FR-041's Parent-facing *data access* is implemented and tested (via `GET /attendance/{classId}`, `GET /results/me`, `GET /fees/me`, `GET /fees/payments/{studentId}`, all Parent-scoped through a manually-supplied `student_id`) — what remains permanently unbuilt is only this dedicated, child-selector-driven aggregation screen. The Milestone 10 Parent Dashboard (`UI_Wireframes.md` Section 2) provides a partial substitute — Fee Status and Recent Results via the same manual-`student_id` pattern — for callers who already know their linked child's ID.

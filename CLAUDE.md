...
- **`backend/.env` and `frontend/.env` are developer-local and off-limits, full stop.** Never delete, overwrite, recreate, rename, or clean them, and never run a workspace-sync/cleanup command that would remove them (`git clean -fd`/`-fdx`/`-xdf` or equivalent are never run in this repo, for any reason). If a task needs an environment variable added/changed, update `.env.example` only and tell the user exactly what to copy into their local `.env` — never write to their `.env` directly. If verification work needs real env values (e.g., testing app boot/config validation), do it in an isolated `git clone` temp directory (see Section 10/`MILESTONE_VERIFICATION_CHECKLIST.md`) or via inline shell env vars (`DATABASE_URL=... command`), never by touching the `.env` file inside the actual project working directory. If a planned workflow would remove or modify a local `.env`, stop and ask for confirmation instead of proceeding.
...
- **`git status` before every commit, no exceptions.** Run it (and inspect the actual diff, not just filenames — `git diff` / `git diff --staged`) immediately before staging and immediately before committing. If a local developer configuration file appears in the staged or unstaged changes unexpectedly — `.env`/`.env.local` and variants, IDE settings (`.vscode/`, `.idea/`, `.vs/`, `.claude/`), personal secrets or credentials, or anything else not already an intended part of the change being made — **stop and ask for confirmation** before committing, staging, or touching that file in any way. "Unexpectedly" means: not something this specific task was asked to change. A file legitimately being edited as part of the current task (e.g., `.env.example` when updating documented env vars) is not "unexpected" and doesn't require asking.
...
13. **Never touch a local `.env` file.** Not `backend/.env`, not `frontend/.env`, not any `.env.local`/environment-specific variant — never create, overwrite, delete, or clean one, even temporarily "for testing" and even if it's regenerated identically afterward. See Section 8 for the full policy and the required alternative (isolated clone or inline env vars) when verification needs real env values.
14. **`git status` immediately before every commit — no exceptions.** If a local developer configuration file (`.env`/variants, IDE settings, personal secrets) shows up staged or unstaged and wasn't an intended target of the current task, stop and ask before proceeding. See Section 8 for the full policy.
